...
public class ManageTask {
	...
	public View initView(Context context, View view, String cfg) {
		...
		RadioButton radioBtn00 = (RadioButton) view.findViewById(R.id.radio00);
		RadioButton radioBtn01 = (RadioButton) view.findViewById(R.id.radio01);
		RadioButton radioBtn02 = (RadioButton) view.findViewById(R.id.radio02);
		
		try {
			JSONObject jsonObj = new JSONObject(cfg);
			int whichBtn = jsonObj.getInt("logintype");//0=>电脑，1=>Android，2=>WAP
			switch (whichBtn) {
				case 0:
					radioBtn00.setChecked(true);
					break;
				case 1:
	...
	public boolean dataValidator(Context context, View view) {
		boolean flag = false;
		//检测百度登录方式的RadioGroup，必须有选项被选中才返回true
		RadioGroup rg = (RadioGroup) view.findViewById(R.id.rgLoginType);
		int selectedRD = rg.getCheckedRadioButtonId();
		if (selectedRD == R.id.radio00 || selectedRD == R.id.radio01 || selectedRD == R.id.radio02) {
			flag = true;
		}
		return flag;
	}
